...
En pratique, cette structure est implémentée en utilisant la technique de l'union par rang avec compression de chemin. Avec elles sont définies deux fonctions auxiliaires :
   - `find_root`, pour trouver la racine d'un ensemble dans la structure.
   - `union_sets`, pour fusionner deux ensembles dans la structure.
...
Dans cette section, nous allons discuter de la **simulation de données** pour tester notre package Rcpp. Nous avons commencé par simuler des données pour un **petit graphe** avec seulement 5 sommets (n=5).
...
Ici, nous allons analyser différents cas particuliers de graphes et leur impact sur les performances de nos fonctions. Le **graphe disjoint** est un cas particulier car il est composé de plusieurs composantes connexes séparées, et donc de plusieurs sous-graphes.

Une application possible d'un ACM sur un graphe disjoint est la planification de réseaux de communication pour des entreprises ayant plusieurs bureaux situés dans différentes villes. Dans ce cas, chaque composante connexe représente un sous-réseau de bureaux connectés les uns aux autres, et l'ACM peut être utilisé pour déterminer le chemin le plus court pour connecter tous les sous-réseaux ensemble, ce qui permet de minimiser les coûts de construction et de maintenance du réseau.
...
Comme expliqué dans la présentation des algorithmes, l'ACM n'est pas initialement fait pour les graphes non connexes : c'est pour cela que les algorithmes ne s'appliquent pas.

### Graphe cycle complet

Un **graphe cycle complet** est un cas particulier de graphe où tous les sommets sont connectés les uns aux autres en formant un cycle.
